...
--
-- audio_path points at the shared `audios/default/` folder in the `flash-app`
-- bucket (files generated by `make test-audio`, uploaded by `make audio.upload`).
-- It is derived from english, matching the file names in
-- `supabase/assets/start_audios`.
--
-- Tags are thematic and feed the "by tag" breakdown on the progress screen.
-- =============================================================================

insert into public.flashcards (created_by, english, portuguese, phonetic, example, tags, audio_path)
select null, s.english, s.portuguese, s.phonetic, s.example, s.tags,
  'audios/default/audio-' || regexp_replace(s.english, '[^a-zA-Z0-9]', '_', 'g') || '.mp3'
from (values
...
-- Backfill audio_path on global starter cards that were seeded before the
-- column existed (the guarded insert above only creates missing rows). Path is
-- derived from english so it matches the files in supabase/assets/start_audios.
update public.flashcards
set audio_path = 'audios/default/audio-' || regexp_replace(english, '[^a-zA-Z0-9]', '_', 'g') || '.mp3'
